...
class ZcspiderPipeline(object):
    ...
    def _conditional_insert(self, tx, item):
        try:
            ...
            sql = "INSERT INTO `zu`.`news` (`url`, `title`, `time`, `cid`) VALUES (%s,%s,%s,%s);"
            params = (item["url"], item["title"],item["time"],self.cid)
            ...
            sqlcon = "INSERT INTO `zu`.`news_content` (`id`, `content`) VALUES (%s,%s);"
            params = (self.cid, item["content"])
            tx.execute(sqlcon, params)

            self.cid += 1
            # self.dbpool.commit()

        except MySQLdb.Error, e:
            print "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++failue++++"+str(e.message)+str(e.args)
